...
INSERT INTO genders (code, display_name, sort_order) VALUES
    ('male', 'Male', 1),
    ('female', 'Female', 2),
...
CREATE TABLE IF NOT EXISTS participants (
    ...
    public_id UUID NOT NULL DEFAULT gen_random_uuid() UNIQUE, -- external/public ID
    ...
    CONSTRAINT chk_active_username UNIQUE (username) WHERE is_deleted = false
);
...
-- =====================================================================
-- ATTENTION CHECKS (one active per image)
-- =====================================================================
...
-- Enforce only one active check per image
CREATE UNIQUE INDEX idx_attention_active_unique
...
CREATE TABLE IF NOT EXISTS reward_winners (
    ...
    reason_code VARCHAR(60), -- e.g. 'top_priority_score', 'random_lottery', 'manual'
...
-- Participants: force RLS – even superusers must respect unless explicitly bypassed
...
-- Submissions: only the owning participant can read/write their own rows
